...
**** Binds
     ...
     ;; handy binds for increasing and decreasing font size on the fly
     ;; font sizes, taking into account i3wm keybinds
...
***** Binds
      ...
      (define-key seconf-window-map (kbd "=") 'shrink-window-horizontally)
      (define-key seconf-window-map (kbd "+") 'enlarge-window-horizontally)
      (define-key seconf-window-map (kbd "-") 'shrink-window)
      (define-key seconf-window-map (kbd "_") 'enlarge-window)
...
***** Customize
      ...
      (customize-set-variable 'verb-auto-show-headers-buffer nil)
      ...
      ;; stop creating new HTTP Response Buffers
...
***** Hooks
      ...
      This hook is a bit too disruptive, however, it can be useful
...
***** Binds
      ...
      This keybind, even residing in the ==verb== section, is more
      useful than the last assigned command, which was
      ==org-babel-check-src-block==.

      #+BEGIN_SRC emacs-lisp

      (eval-after-load 'verb
        (lambda()
          ;; add a more sane keybind to ob
          (when (boundp 'org-babel-map)
            (define-key org-babel-map (kbd "C-c") 'org-babel-execute-src-block))

          ;; don't ask for confirmation when executing verb code blocks
          (customize-set-variable 'org-confirm-babel-evaluate
                                  (lambda (language code-block)
                                    (if (string= language "verb")
                                        nil t)))

          ;; add the verb dialect to ob
      ...
      (eval-after-load 'org
        (lambda()
          (when (boundp 'org-mode-map)
            ...
            (define-key org-mode-map (kbd "C-c C-x C-v") 'org-display-inline-images))))
...
**** Chicken-mode
     ...
     The major mode for editing Chicken Scheme code.

***** Require

      #+BEGIN_SRC emacs-lisp

      ;; (require chicken-mode nil t)
...
***** Functions
      ...
      (defun seconf/chicken/set-company-backends ()
        "Set go company backends."
        (interactive)
        (seconf-set-company-backends
         '((company-dabbrev-code :with
                                 company-dabbrev
                                 company-yasnippet)
           (company-files))))
...
***** Hooks
      ...
      (add-hook 'chicken-mode-hook
                (lambda ()
                  ;; set dash docsets
                  ;; (seconf-activate-docset '"Chicken")
                  ;; set company backends
                  (seconf/chicken/set-company-backends)))
...
***** TODO Enable
      ...
      ;; add (*.scm . chicken-mode) to auto-mode-alist
      ;; init chicken-mode when a file with the extersion .scm is opened
      ;; (add-hook 'window-setup-hook
      ;;           (lambda ()
      ;;             (add-to-list 'auto-mode-alist '("\\.scm\\'" . chicken-mode))))
...
***** Binds
      ...
      (eval-after-load 'chicken-mode
              (lambda ()
                (when (boundp 'chicken-op-mode-map)
                  (define-key chicken-op-mode-map (kbd "TAB") 'seconf-complete-in-buffer-or-indent))))
...
**** clojure-mode
     ...
     The major mode for editing Clojure code.
...
***** Functions
      ...
      (defun seconf/clojure/set-company-backends ()
        "Set go company backends."
        (interactive)
        (seconf-set-company-backends
         '((company-dabbrev-code :with
...
***** Hooks
      ...
      (add-hook 'clojure-mode-hook
                (lambda ()
                  ;; set dash docsets
                  (seconf-activate-docset '"Clojure")
                  ;; set company backends
                  (seconf/clojure/set-company-backends)))

      (eval-after-load 'clojure-mode
        (lambda ()
          (when (boundp 'clojure-mode-map)
            (define-key clojure-mode-map (kbd "TAB") 'seconf-complete-in-buffer-or-indent))))

      #+END_SRC

**** iclj-mode

     A minor mode for fast REPL interactions for Clojure code.
...
***** Functions
      ...
      (defun seconf/js/set-company-backends ()
...
***** Hooks
      ...
      (add-hook 'js-mode-hook
                (lambda ()
                  ;; set company backends
                  (seconf/js/set-company-backends)

                  ;; set dash docsets
                  (seconf-activate-docset '"JavaScript")))
...
***** Enable
      ...
      ;; add (*.js . js-mode) to auto-mode-alist
      ;; init js-mode when a file with the extersion .js is opened
      (add-hook 'window-setup-hook
                (lambda ()
      ...
      (eval-after-load 'js
        (lambda ()
          (when (boundp 'js-mode-map)
            (define-key js-mode-map (kbd "TAB") 'seconf-complete-in-buffer-or-indent))))
...
**** IPython
     ...
     For now, this section holds only information needed to edit
     ipython source code blocks in an org file, alongside
     ==ob-ipython==.
